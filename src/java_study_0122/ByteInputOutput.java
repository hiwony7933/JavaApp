...
public class ByteInputOutput {
	...
	public static void main(String[] args) {
		...
		try (
				...
				FileOutputStream fos = new FileOutputStream("./src/java_study_0122/file.dat");

				// 파일에서 읽어오기 위한 인스턴스 생성
				FileInputStream fis = new FileInputStream("./src/java_study_0122/file.dat");
			...
			/*
			 //한번에 읽기 //읽을 데이터 개수를 가져오기 int len = fis.available(); //읽을 데이터를 저장할 배열을 생성 b=
			 new byte[len]; //데이터 읽기 fis.read(b); //바이트 배열을 문자열로 변환 String data = new
			 String(b); System.out.println(data);
			 */ // 한번에 읽으면 메모리 터짐

			// 용량이 큰 경우 한번에 읽으면 메모리 부족으로 예외가 생기거나 스긴이 오래 걸릴 수 있습니다.
			// 나누어서읽는 것이 효율적

			// 저장할 배열을 생성 - 크기는 8의 배수로 생성 
