	...
	public static void main(String[] args) {
		...
		try {
			...
			*/
			...
			/* an_txt class 가져오기 
			Elements an_text = document.getElementsByClass("an_txt");
			//가져온 데이터르 ㄹ순회하면서 출력 
			for(int i =0 ; i < an_text.size(); i=i+1) {
				Element element = an_text.get(i);
				System.out.println(element.text());
			}			
			*/
			//선택자이용
			...
			Elements span = document.select("#PM_ID_themecastBody > div > div > div > ul > li:nth-child(13) > a > span.td_tw > span");
			//가져온 데이터르 ㄹ순회하면서 출력 
			for(int i =0 ; i < span.size(); i=i+1) {
				Element element = span.get(i);
				System.out.println(element.text());
