...
public class KakoOpenAPI {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 헤더 설정
			con.addRequestProperty("Authorization", "KakaoAK 0ae14c42ae0198bceb42b4e627d3464f ");
		...
		if (json == null) {
			...
			// 전체 문자열을 JSON에 객체로 변환
			JSONObject root = new JSONObject(json);
			// System.out.println(root);

			// meta키의 내용을 JSONObject 로 가져오기
			...
			for (int i = 0; i < len; i = i + 1) {
				...
				try {
					...
					System.out.println(title + " : " + price + "원" /* + URLDecoder.decode(thumb,"utf-8") */);
				} catch (Exception e) {
				}
