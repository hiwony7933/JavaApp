...
public class DoubleArray {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < players.size(); i = i + 1) {
			...
			ArrayList<String> imsi = players.get(i);	// imsi 의 데이터를 하나씩 temp에 대입
		...
		/*
		 * // 전체 리스트를 하나하나의 리스트로 imsi에 대입하고 for (ArrayList<String> imsi : players) { //
		 * imsi의 데이터를 하나씩 temp에 대입 for (String temp : imsi) { System.out.print(temp +
		 * "\t"); } System.out.println(" "); }
		 */
		System.out.println("---------------------------------------");
		
		//앞에서의 문제는 list의 list를 만들때 각 list의 특징을 같이 저장하지 못한다는데 있습니다. 
		//팀이름은 문자열이고 팀의선수명단은 배열이라서 같이 list에 저장을 몬합니다. (동일한데이터만저장가능함)
