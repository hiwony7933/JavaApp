...
public class ParallelMain {
	...
	public static void main(String[] args) {
		...
		// 일반 스트림을 생성해서 ar에 work를 수행 - 10초정도 소요
		long start = System.currentTimeMillis();

		arStream.forEach((data) -> {work(data);});

		long end = System.currentTimeMillis();
		System.out.println("순서처리:" + (end - start));

		// 병렬스트림을 생성해서 work를 수행 - cpu 코어 개수에 따라 걸리는 시간이 다름
		// 코어가 5개 이상이면 한번에 수행하므로 2초 정도 소요
		// 데이터의 모임을 가지고 작업을 수행하는 경우 다른 데이터의 영향을 전혀 받지않고 수행되는 경우에는 병렬처리를하는것이 시간으로는 이득이다.
		// 앞의 데이터의 연산 결과를 가지고 다음 작업을 수행하는 작업은 병렬처리 하는것이 곤란 
